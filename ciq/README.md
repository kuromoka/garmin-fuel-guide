...
Set `relayUrl`, `relayToken`, and `relayIntervalSeconds` through the Connect IQ app settings. The URL and token default to empty, so no network request is made until configured. The field always sends `temperatureC: null`.
