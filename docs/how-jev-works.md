...
flowchart TD
    ...
    C["表情・色・ポーズを切り替える<br/>画面に雰囲気名と風向きを表示"]
...
自分で用意する中継サーバーはありません。スマホは通信を橋渡しし、判定はJevのサーバー、描画はウォッチが担当します。通信OFFでは灰色のキャラクターに固定し、風の矢印だけをローカルで表示します。
...
| Choiceの値 | Jevへ渡している基準 | ウォッチの見た目 |
| --- | --- | --- |
| calm | low motion or relaxed rhythm | 青い体、目を細めた顔 |
| steady | consistent motion and rhythm | 緑の体、交互に動く足 |
| bouncy | lively or changing rhythm | 黄色い体、上げた手、上下の動き |
| focused | sustained purposeful movement | ピンクの体、寄せた眉、動く足 |
...
応答のconfidenceはキャラクター選択に対するJevの確信度です。画面には表示しませんが、0〜1の数値であることを検証します。健康状態の確率ではありません。

## 画面の表示

画面には大きなキャラクター、風向き、キャラクターの雰囲気名だけを表示します。通信状態、確信度、天気の数値、観測からの経過時間、距離・タイム・ペース・心拍数は表示しません。有効なJevの結果がないときは、灰色で目を閉じた `RESTING`（日本語では「おやすみ」）にします。
...
天気は60秒ごとにキャッシュを読み直し、観測時刻からの経過時間を毎回計算します。風速0.1m/s未満では矢印を消します。観測時刻が不明・未来・6時間超の場合や、方向が欠けている場合も矢印を出しません。天気の数値と観測の古さはStateに残します。天気の数値や待機状態の文字は画面へ出しません。
...
- 成功した結果は、アクティビティのタイマーで180秒経つと古い結果として扱い、灰色で目を閉じたキャラクターへ戻します。
- 通信OFFではAPIを呼びません。表情の切り替えや足の動きもウォッチ側で描画するため、フレームごとのAPI呼び出しはありません。
